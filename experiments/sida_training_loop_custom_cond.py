...
class DistributionMatchingLoss(nn.Module):
    ...
    def forward(self, real_images, generated_images, labels=None):
        """
        Args:
            real_images: 真实图像 [B, C, H, W]
            generated_images: 生成图像 [B, C, H, W]
            labels: 标签 [B, label_dim] 或 None (无条件生成)
        """
        # 如果没有标签或标签维度为0，使用原来的方法（全局matching）
        if labels is None or labels.shape[1] == 0:
            return self._compute_loss_global(real_images, generated_images)
        
        # 有标签的情况：按类别分组计算loss
        return self._compute_loss_per_class(real_images, generated_images, labels)
    
    def _compute_loss_global(self, real_images, generated_images):
        """全局匹配（无条件生成）"""
        total_loss = 0.0
    ...
    def _compute_loss_per_class(self, real_images, generated_images, labels):
        """按类别匹配（条件生成）"""
        # 将labels转换为类别索引
        # 假设labels是one-hot或multi-hot编码，转换为类别ID
        if labels.dim() > 1:
            # 如果是one-hot，找到最大值的索引
        ...
        for label in unique_labels:
            ...
            real_images_class = real_images[mask]
            gen_images_class = generated_images[mask]
            
            # 对当前类别计算loss
            class_loss = 0.0
            for net in self.networks:
                real_emb = net(real_images_class)
                gen_emb = net(gen_images_class)
                
                mmd = self._compute_mmd(real_emb, gen_emb)
                class_loss += mmd
            
            total_loss += class_loss / len(self.networks)
            num_valid_classes += 1
                ...
                with misc.ddp_sync(G_ddp, (round_idx == num_accumulation_rounds - 1)):
                    ...
                    if use_gan is False:    
                        ...
                        distribution_loss_fn = DistributionMatchingLoss(
                            input_shape=(3, 64, 64),  # 对于RGB 64×64图片
                            num_networks_per_type=1,  # 每种类型1个网络，共4个
                            embed_dim=64
                        ).to(device)
                        loss=loss.sum().mul(1 / batch_gpu_total)
                        EL_loss = distribution_loss_fn(real_images, images, labels=labels)  # 添加labels参数
                        loss += EL_loss * 10
                    
                    loss=loss.mul(loss_scaling_G)
